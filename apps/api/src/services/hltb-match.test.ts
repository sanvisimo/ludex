...
const hit = (over: Partial<HltbSearchHit> & { hltbId: number; name: string }): HltbSearchHit => ({
  alias: null,
  type: "game",
  releaseYear: null,
  ...over,
...
const residentEvil4 = [
  ...
  hit({ hltbId: 137868, name: "Resident Evil 4 - Separate Ways", type: "dlc", releaseYear: 2023 }),
  hit({ hltbId: 66855, name: "Resident Evil 4: Separate Ways", type: "dlc", releaseYear: 2005 }),
];
...
describe("titleSimilarity", () => {
  it("distingue un titolo dallo stesso titolo con un pezzo attaccato", () => {
    ...
    const edizione = normalizeTitle("The Witcher 3: Wild Hunt - Game of the Year Edition");
...
describe("rankHltbCandidates", () => {
  ...
  it("butta i DLC, che affollano ogni ricerca", () => {
    const ranked = rankHltbCandidates({ name: "The Witcher 3: Wild Hunt", releaseYear: 2015 }, witcher3);
  ...
  it("guarda anche l'alias, non solo il nome principale", () => {
    const ranked = rankHltbCandidates({ name: "Hollow Knight: Voidheart Edition", releaseYear: 2017 }, [
      hit({
        hltbId: 26286,
        name: "Hollow Knight",
...
describe("pickByName", () => {
  it("preferisce il titolo identico a quello che gli somiglia tantissimo", () => {
    ...
    const ranked = rankHltbCandidates({ name: "Orcs Must Die!", releaseYear: 2011 }, [
  ...
  it("non basta il titolo identico se l'anno è di un altro gioco", () => {
    ...
    const ranked = rankHltbCandidates({ name: "7 Days to Die", releaseYear: 2024 }, [
  ...
  it("non aggancia niente quando nessuno somiglia abbastanza", () => {
    const ranked = rankHltbCandidates({ name: "Un Gioco Che Non Esiste", releaseYear: 2020 }, witcher3);
...
describe("shortenTitle", () => {
  it("taglia il sottotitolo dopo i due punti e la barra", () => {
    // I due casi veri: senza il taglio HLTB non restituisce niente, perché cerca
    // in AND su tutti i termini.
    expect(shortenTitle("The Witcher: Enhanced Edition Director's Cut")).toBe("The Witcher");
    expect(shortenTitle("Rising Storm/Red Orchestra 2 Multiplayer")).toBe("Rising Storm");
  });
