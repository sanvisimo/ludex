...
describe('enrichGameFromHltb', () => {
  ...
  it("riconosce il gioco anche sull'appid alternativo", async () => {
    ...
    mockedSearch.mockResolvedValue([
      hltbHit({ hltbId: 1066, name: 'BioShock 2', releaseYear: 2010 }),
    ]);
    mockedDetail.mockResolvedValue(
      hltbDetail({
        hltbId: 1066,
        name: 'BioShock 2',
        steamAppIds: ['409720', '8850'],
      }),
    );
  ...
  it('riprova col titolo accorciato quando la ricerca non torna niente', async () => {
    ...
    mockedDetail.mockResolvedValue(
      hltbDetail({ hltbId: 10267, name: 'The Witcher' }),
    );
  ...
  it("non riprova quando l'anno non lo sappiamo", async () => {
    ...
    // Un titolo accorciato apre a candidati che col nome intero non sarebbero
    // mai comparsi: senza anno non resterebbe niente a verificarli.
  ...
  it('come terzo tentativo prova la coda dopo i due punti', async () => {
    const game = await createGame({
      name: 'Gabriel Knight 3: Blood of the Sacred, Blood of the Damned',
      firstReleaseDate: anno(1999),
    });
    // Né il titolo intero né la testa trovano niente: "3" non è "III".
    mockedSearch.mockResolvedValueOnce([]);
    mockedSearch.mockResolvedValueOnce([]);
    mockedSearch.mockResolvedValueOnce([
      hltbHit({
    ...
    mockedDetail.mockResolvedValue(
      hltbDetail({ hltbId: 3783, name: 'Gabriel Knight III' }),
    );

    const esito = await enrichGameFromHltb(game.id);

    expect(mockedSearch).toHaveBeenNthCalledWith(2, 'gabriel knight 3');
    expect(mockedSearch).toHaveBeenNthCalledWith(
      3,
      'blood of the sacred blood of the damned',
    );
    expect(esito).toMatchObject({ status: 'ok', hltbId: 3783 });
  });

  it('dice tutti i titoli accorciati provati quando falliscono tutti', async () => {
    const game = await createGame({
    ...
    // Senza, dalla admin sembrerebbe che i tentativi non siano stati fatti.
    const errore = (await sourceRow(game.id))?.error;
    expect(errore).toContain('Gabriel Knight 3');
    expect(errore).toContain('Blood of the Sacred');
  });
  ...
  it('segna not_found coi candidati scartati, quando nessuno convince', async () => {
    ...
    expect(esito).toMatchObject({
      status: 'not_found',
      reason: 'candidati non convincenti',
    });
  ...
  it('salta un gioco inesistente', async () => {
    const esito = await enrichGameFromHltb(
